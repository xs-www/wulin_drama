...
def updateDb(db_path=None):
    ...
    sql_files = sorted(SQL_DIR.glob("database_dump_new.sql"))
...
class CharacterDao:
    ...
    def get_next_id(self, conn):
        ...
        """
        cursor = conn.cursor()
        cursor.execute("SELECT seq as max_id FROM sqlite_sequence WHERE name = 'Character'")
        row = cursor.fetchone()
    ...
    def update_character(self, char_id, values: list, conn):
        """
        更新角色信息
        :param char_id: 角色ID
        ...
        """
        cursor = conn.cursor()
        cursor.execute(self.mapper.get("update_query"), (*values, char_id))
...
class FetterDao:
    ...
    def delete_fetter(self, fetter_id, numofpeople, conn):
        """
        删除羁绊
        :param fetter_id: 羁绊ID
        :param numofpeople: 参与人数
        """
        cursor = conn.cursor()
        cursor.execute("DELETE FROM fetter WHERE id = ? AND numofpeople = ?", (fetter_id, numofpeople))
